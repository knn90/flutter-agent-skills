# Refactor candidates — Dart

> Reference for [`flutter-tdd`](../SKILL.md). Dart-native rewrite of mattpocock/skills `tdd/refactoring.md` (MIT).
> The REFACTOR step of the cycle.

After a test goes green, scan the new (and surrounding) code for these smells:

| Smell | Fix |
|---|---|
| **Duplication** | Extract a function / class; lift shared setup |
| **Long method** | Break into `_private` helpers — keep tests on the **public** interface, not the helpers |
...
| **Primitive obsession** (`String`/`int` carrying domain meaning) | Introduce a value type — an extension type, a `sealed`/`enum` status, a typed id |
...
- **Don't change behavior here.** New behavior needs a new failing test first (back to RED) — refactoring is
  structure-only.
- **Test through the public interface**, so extracting helpers/splitting classes doesn't touch a single test
  (if it does, the tests were coupled to implementation — see [tests.md](tests.md)).
- For SOLID/value-object/deep-module judgement calls, defer to `flutter-solid-expert`; for widget-specific
  restructuring (extracting widgets, const, rebuild scope), `flutter-widget-expert`.
